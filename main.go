...
func (w Walking) Calories() float64 {
	...
	doubleSpeed := math.Pow(speed, 2) //возведение скорости в квадрат
	return ((CaloriesWeightMultiplier*w.Training.Weight + (doubleSpeed/w.Weight)*CaloriesSpeedHeightMultiplier*w.Training.Weight) * w.Training.Duration.Hours() * MinInHours)
}
...
func ReadData(training CaloriesCalculator) string {
	info := training.TrainingInfo()     // получаем информацию о тренировке
	info.Calories = training.Calories() // получаем количество затраченных калорий
